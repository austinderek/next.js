...
describe('typed-routes-validator', () => {
  ...
  it('should generate route validation correctly', async () => {
    ...
    expect(dts).toMatch(
      /const handler = {} as typeof import\(".*\/app\/page"\)\s+handler satisfies AppPageConfig<"\/">/
    )
    expect(dts).toMatch(
      /const handler = {} as typeof import\(".*\/app\/send-email\/route"\)\s+handler satisfies RouteHandlerConfig<"\/send-email">/
    )
    expect(dts).toMatch(
      /const handler = {} as typeof import\(".*\/pages\/about"\)\s+handler satisfies PagesPageConfig/
    )
    expect(dts).toMatch(
      /const handler = {} as typeof import\(".*\/pages\/api\/test-route"\)\s+handler satisfies ApiRouteConfig/
    )
    expect(dts).toMatch(
      /const handler = {} as typeof import\(".*\/app\/layout"\)\s+handler satisfies LayoutConfig<"\/">/
    )
  })
