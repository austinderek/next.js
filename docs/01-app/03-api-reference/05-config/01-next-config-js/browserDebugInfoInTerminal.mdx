...
> - When enabled, it works in development mode only.
...
When browser logs are serialized for the terminal, circular references are handled safely and displayed as `[Circular]`.
...
Source locations are included by default when this feature is enabled. To suppress them, set `showSourceLocation: false`.
